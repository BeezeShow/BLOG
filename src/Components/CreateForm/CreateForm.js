...
const CreateForm = () => {
  ...
  return isLoad ? (
    ...
    <Form className="login-form" form={form} name="register" onFinish={onFinish} scrollToFirstError>
      ...
      {err.username ? (
        <div style={{ color: 'red', position: 'absolute', top: 270 }}>{'username is alredy taken'}</div>
      ) : null}
      Email address
      <Form.Item
        name="email"
      ...
      {err.email ? <div style={{ color: 'red', position: 'absolute', top: 350 }}>{'email is alredy taken'}</div> : null}
      Password
      <Form.Item
        name="password"
        rules={[
